function newVals=dirichletboundary_new(L,index,vals,p,gma)
...
antiIndex(index)=[];
%%%Find RHS
b=-L(antiIndex,index)*(vals)+gma*p(antiIndex,:);        
lam = sum(p(antiIndex,:),2);
lamda = sparse_new(lam);
% W=sparse([edges(:,1);edges(:,2)],[edges(:,2);edges(:,1)], ...
%     [weights;weights],N,N);

% a=diag(L(antiIndex,antiIndex));
% f=diag(a,0);                                           
%%%%Solve system
x=(L(antiIndex,antiIndex)+gma*lamda)\b;
